    ...
    <div>
      ...
      <div className="max-w-2xl space-y-5">
        ...
        <Card className="p-6">
          ...
          />
          <ToggleRow
            label={t("settings.academicAlerts")}
            on={settings.academic}
            onClick={() => toggle("academic")}
          />
          <ToggleRow
            label={t("settings.serviceUpdates")}
            on={settings.service}
...
function ToggleRow({ label, on, onClick, last }) {
  return (
    <div
      className={`flex items-center justify-between py-3.5 ${!last ? "border-b border-line-soft" : ""}`}
    >
      <strong className="text-sm text-ink-900">{label}</strong>
      <button
        onClick={onClick}
