...
const markdownComponents = {
  ...
  h1: ({ children }) => (
    <h3 className="text-base font-bold text-ink-900 mt-4 mb-2 first:mt-0">
      {children}
    </h3>
  ),
  h2: ({ children }) => (
  ...
  h3: ({ children }) => (
    <h4 className="text-sm font-bold text-ink-900 mt-3 mb-1.5 first:mt-0">
      {children}
    </h4>
  ),
  ul: ({ children }) => (
    <ul className="list-disc ps-5 space-y-1 mb-3">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal ps-5 space-y-1 mb-3">{children}</ol>
  ),
  li: ({ children }) => (
    <li className="text-sm text-ink-800 leading-relaxed">{children}</li>
  ),
  a: ({ href, children }) => (
    <a
      href={href}
  ...
  code: ({ className, children, ...props }) => {
    ...
    return (
      <code
        className="bg-sand-100 text-navy-800 rounded px-1.5 py-0.5 text-xs font-mono"
        {...props}
      >
        {children}
      </code>
  ...
  pre: ({ children }) => (
    <pre className="bg-navy-950 text-sand-50 rounded-md p-3.5 overflow-x-auto text-xs font-mono mb-3">
      {children}
    </pre>
  ),
  blockquote: ({ children }) => (
    <blockquote className="border-s-4 border-line ps-3.5 italic text-ink-700 mb-3">
      {children}
    </blockquote>
  ),
...
const STRINGS = {
  ...
  poweredBy: {
    en: "Powered by UB· grounded in ub.edu.sa",
    ar: "مدعوم بواسطة Claude · بالاستناد إلى ub.edu.sa",
  },
  offlineBadge: {
    en: "Offline quick-answer (AI service unavailable)",
    ar: "إجابة سريعة دون اتصال (خدمة الذكاء الاصطناعي غير متاحة)",
  },
  fallbackHint: {
    en: "Showing an offline quick answer instead.",
    ar: "يتم عرض إجابة سريعة دون اتصال بدلاً من ذلك.",
  },
};
...
const SUGGESTIONS = [
  ...
  {
    en: "Suggest courses or workshops for me",
    ar: "اقترح لي مقررات أو ورش عمل",
  },
  ...
  {
    en: "When is my monthly allowance paid?",
    ar: "متى تُصرف المكافأة الشهرية؟",
  },
    ...
    <div>
      <div className="mb-6 flex items-center gap-3.5">
        <img
          ...
          alt=""
          className="w-12 h-12 rounded-full object-cover ring-2 ring-teal-500/50 shrink-0"
        />
        <div>
          ...
          <Card className="p-6 animate-fade-in-up">
            <div className="flex items-start gap-3.5 pb-5 mb-5 border-b border-line-soft">
              <img
                src={ASSISTANT_AVATAR}
                alt=""
                className="w-11 h-11 rounded-full object-cover ring-2 ring-teal-500/40 shrink-0"
              />
              <div>
        ...
        {!thinking && offlineResult && (
          <Card className="p-6 animate-fade-in-up">
            ...
            <p className="text-sm text-ink-700 leading-relaxed mb-5">
              {L(offlineResult.desc)}
            </p>

            {offlineResult.checks.length > 0 && (
              <div className="mb-5">
                <h6 className="text-xs font-bold uppercase tracking-wide text-ink-500 mb-2.5">
                  {t("ai.eligibility")}
                </h6>
                {offlineResult.checks.map((c, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2.5 text-sm text-ink-900 py-1"
                  >
            ...
            <div className="flex flex-wrap gap-2.5">
              {offlineResult.regulationId && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => openRegulation(offlineResult.regulationId)}
                >
                  <ScrollText size={16} />
                  {t("ai.viewRegulation")}
                </Button>
        ...
        {!thinking && notUnderstood && (
          <Card className="p-8 text-center animate-fade-in-up">
            ...
            <h3 className="text-base font-bold text-ink-900 mb-1.5">
              {t("ai.notUnderstood.title")}
            </h3>
            <p className="text-sm text-ink-500 mb-5">
              {refusalNote || t("ai.notUnderstood.body")}
            </p>
            <div className="flex items-center justify-center gap-2.5 flex-wrap">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => navigate("/app/services/it-helpdesk")}
              >
